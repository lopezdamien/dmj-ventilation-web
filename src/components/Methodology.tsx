                ...
                <div className="relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-12 relative z-10">
                        {steps.map((step, index) => (
                            ...
                            >
                                <div className="w-12 h-12 rounded-full bg-white border border-gray-200 flex items-center justify-center text-primary group-hover:border-primary group-hover:bg-primary group-hover:text-white transition-all duration-300 mb-8 z-10">
                                    <step.icon size={20} strokeWidth={1.5} />
                                </div>
                                <h3 className="text-base font-bold uppercase tracking-wide text-primary mb-3">{index + 1}. {step.title}</h3>
                                <p className="text-xs text-gray-500 font-medium max-w-[140px] mx-auto leading-relaxed">{step.description}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
